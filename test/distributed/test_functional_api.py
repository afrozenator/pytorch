...
class TestTraceableCollectives(MultiThreadedTestCase):
    ...
    @parametrize("device", ["cpu", "cuda"])
    def test_all_reduce_tensor(self, device):
        print(f'@@@ test_all_reduce_tensor is being called with {device=}')
        if device == "cuda":
            if torch.cuda.device_count() < self.world_size:
                self.skipTest("Not enough CUDA devices")
            torch.cuda.set_device(dist.get_rank())
        ...
        for mesh in [mesh_1d, mesh_2d]:
            ...
            input_tensor = torch.ones(output_size, device=device)
            res_num = 1 * group_size
            print(f'@@@ test_all_reduce_tensor is being called with {input_tensor=}')
            ar_tensor = ft_c.all_reduce_tensor(
                input_tensor, "sum", group=(mesh, 0)
            )
            print(f'@@@ test_all_reduce_tensor output {ar_tensor=}')
            self.assertEqual(ar_tensor, torch.ones(input_size) * res_num)
            # dims_to_scatter = [0, 1]
            # for dim in dims_to_scatter:
            #     group_size = mesh.size(0)
            #     input_size = [3, 3]
